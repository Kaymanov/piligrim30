  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitQuizLead({
        ...
        message: [
          `Сумма долга: ${answers.debt_amount || "—"}`,
          `Просрочки: ${answers.has_overdue || "—"}`,
          `Исполнительные производства: ${answers.has_enforcement || "—"}`,
          `Имущество: ${answers.has_property || "—"}`,
          `Ипотека: ${answers.has_mortgage || "—"}`,
          `Доход: ${answers.income_type || "—"}`,
        ].join("\n"),
